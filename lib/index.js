(function() {
  ...
  module.exports.findPackageDependencies = function(opt) {
    var collection, cwd, originalPathMap, pathMap, pkg, stream;
    if (opt == null) {
      opt = {};
    }
    ...
    async.each(Object.keys(pkg.dependencies || []), function(dep, cb) {
      var converted, depDir, depFile, depPkg, pushStream, ref;
      ...
      if (fs.statSync(depDir).isDirectory()) {
        ...
        pushStream = function(dep, depFile, newFile) {
          var contents, depFileDir, sourceMap, sourceMapFile, sourceMapPath, version;
        ...
        if (converted) {
          if (typeof converted === 'function') {
            converted = converted(function(err, file) {
              if (err) {
                return cb(err);
              }
            ...
            if ((ref = typeof converted) !== 'object' && ref !== 'string') {
              return;
            }
          }
          if (converted && typeof converted === 'object') {
            return async.each(Object.keys(converted).filter(function(item) {
              return item === dep || !collection[item] && (item.indexOf(dep + '/') === 0 || item.indexOf(dep + '-') === 0 || item.indexOf(dep + '_') === 0);
            }), function(dep, cb) {
              if (opt.ignore && opt.ignore.indexOf(dep) >= 0) {
                return cb();
              }
